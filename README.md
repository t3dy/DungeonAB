...
**▶️ Play v4.4: https://dungeon-ab.vercel.app** &nbsp;·&nbsp; [v3.0](https://dungeon-ab.vercel.app/v3/) &nbsp;·&nbsp; [v2.0](https://dungeon-ab.vercel.app/v2/) &nbsp;·&nbsp; [v1.0](https://dungeon-ab.vercel.app/v1/) &nbsp;·&nbsp; [mirror](https://t3dy.github.io/DungeonAB/)

## Versions

| Version | Play | What it is |
|---|---|---|
| **v4.4** (current) | [dungeon-ab.vercel.app](https://dungeon-ab.vercel.app) | The watchable release. Numbers float over the map as they happen — damage, gold, a trophy, a floor descended — read straight off the run's own record, so a mechanic cannot be added without one. The story panel runs terse: a line a room, with the full account one click away, and clicking a room on the map opens that room's log. Every panel folds, and remembers whether you folded it. Toasts moved off the map and now carry only what a number cannot say. Plus difficulty as choice density: easy deals six cards a pack, nightmare three, with the rounds growing as the pack shrinks so every difficulty drafts the same size pool — the hard part is the choosing, not the having |
| **v4.3** | superseded |  Lock and key: wings sealed behind a door, the key lying back up the passage — or nowhere at all, in which case it wants a rogue, a prybar or Knock, and the last two are heard. Plus the muster: assign drafted kit to particular characters, say who prepares each working (their mind sets its power), rename anybody and write their history. Plus a quartermaster in town selling three pieces of kit priced by what they do. And rooms with room to fight in: chambers half again as big, up to five pieces of furniture apiece, hazards you can put a monster into (pits, floor spikes, a chasm, a lit brazier), the Shove and Pinning tactics that open them to any class, and a camera that frames the room instead of a slab of dungeon. Plus the depth release: a dungeon that goes down — two or three floors joined by stairs, each meaner than the one above; themed wings off the spine; positional combat where the room's shape decides the formation; a class-agnostic tactics tree; two attrition clocks (the lamp and the wound); and a saga that records every state change, saves, and continues into the next delve |
...
Git tags `v1.0`, `v2.0`, `v3.0` and `v4.0` mark the exact sources.
